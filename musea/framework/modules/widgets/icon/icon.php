...
if ( class_exists( 'MuseaCoreClassWidget' ) ) {
	class MuseaElatedClassIconWidget extends MuseaCoreClassWidget {
		public function __construct() {
			parent::__construct(
				'eltdf_icon_widget',
				esc_html__( 'Musea Icon Widget', 'musea' ),
				array( 'description' => esc_html__( 'Add icons to widget areas', 'musea' ) )
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( ! empty( $instance['text_size'] ) ) {
				$text_styles[] = 'font-size: ' . musea_elated_filter_px( $instance['text_size'] ) . 'px';
			}
			...
			if ( ! empty( $instance['icon_text'] ) ) {
				$icon_text_html = '<span class="eltdf-icon-text ' . esc_attr( $icon_text_class ) . '" ' . musea_elated_get_inline_style( $text_styles ) . '>' . esc_html( $instance['icon_text'] ) . '</span>';
			}
			?>
			
			<a <?php musea_elated_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo musea_elated_get_inline_style( $holder_styles ); ?>>
