...
/*
   Class: MuseaElatedClassFramework
...
class MuseaElatedClassFramework {
	...
	private function __construct() {
		$this->eltdOptions         = MuseaElatedClassOptions::get_instance();
		$this->eltdMetaBoxes       = MuseaElatedClassMetaBoxes::get_instance();
		$this->eltdTaxonomyOptions = MuseaElatedClassTaxonomyOptions::get_instance();
		$this->eltdUserOptions     = MuseaElatedClassUserOptions::get_instance();
		$this->eltdDashboardOptions = MuseaElatedClassDashboardOptions::get_instance();
	}
	...
	public function setSkin( MuseaElatedClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class MuseaElatedClassSkinManager
 *
...
class MuseaElatedClassSkinManager {
	...
	/**
	 * @see MuseaElatedClassSkinManager::setSkin()
	 */
	 ...
	 */
	private function setSkin( $skinName = MUSEA_ELATED_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
...
/**
 * Class MuseaElatedClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class MuseaElatedClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( musea_elated_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.eltdSCIcon = '<?php echo musea_elated_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.eltdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'musea' ) ?>';
		</script>
	<?php }
...
class MuseaElatedClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $musea_elated_global_options ) ) {
			return $musea_elated_global_options[ $key ];
...
/*
   Class: MuseaElatedClassAdminPage
   A class that initializes Select Admin Page
*/
class MuseaElatedClassAdminPage implements iMuseaElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new MuseaElatedClassFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iMuseaElatedInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: MuseaElatedClassMetaBoxes
...
/*
   Class: MuseaElatedClassMetaBox
   A class that initializes Select Meta Box
*/
class MuseaElatedClassMetaBox implements iMuseaElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $musea_elated_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: MuseaElatedClassTaxonomyOptions
   A class that initializes MuseaElatedClass Taxonomy Options
*/
class MuseaElatedClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: MuseaElatedClassTaxonomyOption
   A class that initializes MuseaElatedClass Taxonomy Option
*/
class MuseaElatedClassTaxonomyOption implements iMuseaElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new MuseaElatedClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: MuseaElatedClassUserOptions
   A class that initializes MuseaElatedClass User Options
*/
class MuseaElatedClassUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: MuseaElatedClassUserOption
   A class that initializes MuseaElatedClass User Option
*/
class MuseaElatedClassUserOption implements iMuseaElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new MuseaElatedClassUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: MuseaElatedClassDashboardOptions
   A class that initializes MuseaElatedClass Dashboard Options
*/
class MuseaElatedClassDashboardOptions {
	private static $instance;
...
/*
   Class: MuseaElatedClassDashboardOption
   A class that initializes MuseaElatedClass Dashboard Option
*/
class MuseaElatedClassDashboardOption implements iMuseaElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new MuseaElatedClassDashboardFieldFactory();
	}
...
if ( ! function_exists( 'musea_elated_init_framework_variable' ) ) {
	function musea_elated_init_framework_variable() {
		global $musea_elated_global_Framework;
		
		$musea_elated_global_Framework = MuseaElatedClassFramework::get_instance();
		$eltdSkinManager                 = new MuseaElatedClassSkinManager();
		$musea_elated_global_Framework->setSkin( $eltdSkinManager->getSkin() );
	}
	
	add_action( 'musea_elated_action_before_options_map', 'musea_elated_init_framework_variable' );
}
?>