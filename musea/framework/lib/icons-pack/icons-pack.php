...
include MUSEA_ELATED_FRAMEWORK_ICONS_ROOT_DIR . '/icons-pack-interface.php';

foreach ( glob( MUSEA_ELATED_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( musea_elated_is_customizer_item_enabled( $module_load, 'musea_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'musea_elated_get_icon_pack_collections' ) ) {
	function musea_elated_get_icon_pack_collections() {
		$options = apply_filters( 'musea_elated_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'musea_elated_get_icon_pack_options' ) ) {
	function musea_elated_get_icon_pack_options() {
		$options = apply_filters( 'musea_elated_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'musea_elated_get_icon_pack_social_options' ) ) {
	function musea_elated_get_icon_pack_social_options() {
		$options = apply_filters( 'musea_elated_filter_add_icon_pack_into_social_options', $options = array() );
...
/*
 * Class: MuseaElatedClassIconCollections
 */
class MuseaElatedClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = musea_elated_get_icon_pack_collections();
		$this->iconPackOptions       = musea_elated_get_icon_pack_options();
		$this->iconPackSocialOptions = musea_elated_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = musea_elated_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'musea' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'musea' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'musea' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'musea' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'musea' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'musea' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'musea' )       => 'fa-lg',
			esc_html__( 'Small', 'musea' )      => 'fa-2x',
			esc_html__( 'Medium', 'musea' )     => 'fa-3x',
			esc_html__( 'Large', 'musea' )      => 'fa-4x',
			esc_html__( 'Very Large', 'musea' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iMuseaElatedInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'musea' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'musea' ) ] = '';
	...
	public function enqueueStyles() {
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $collection_key => $collection_obj ) {
				wp_enqueue_style( 'musea-select-' . $collection_key, $collection_obj->styleUrl );
			}
		}
	}
