...
if ( ! function_exists( 'musea_elated_add_font_awesome_icon_pack' ) ) {
	function musea_elated_add_font_awesome_icon_pack( $iconCollections ) {
		$iconCollections['font_awesome'] = new MuseaElatedClassIconsFontAwesome( 'Font Awesome', 'fa_icon' );
		
		return $iconCollections;
	}
	
	add_filter( 'musea_elated_filter_add_icon_pack_into_collection', 'musea_elated_add_font_awesome_icon_pack' );
}

if ( ! function_exists( 'musea_elated_add_font_awesome_icon_pack_option' ) ) {
	function musea_elated_add_font_awesome_icon_pack_option( $options ) {
		$options['font_awesome'] = esc_html__( 'Font Awesome', 'musea' );
		
		return $options;
	}
	
	add_filter( 'musea_elated_filter_add_icon_pack_into_options', 'musea_elated_add_font_awesome_icon_pack_option' );
	add_filter( 'musea_elated_filter_add_icon_pack_into_social_options', 'musea_elated_add_font_awesome_icon_pack_option' );
}

class MuseaElatedClassIconsFontAwesome implements iMuseaElatedInterfaceIconCollection {
	public $icons;
	public $title;
	public $param;
	public $styleUrl;
	...
	function __construct($title = "", $param = "") {
		...
		$this->styleUrl = MUSEA_ELATED_FRAMEWORK_ICONS_ROOT . '/font-awesome/css/fontawesome-all.min.css';
	}
	...
	private function getNewIconsArray( $brandsIcons = false ) {
		...
		$iconsArray = array();
		$icons      = json_decode( $wp_filesystem->get_contents( MUSEA_ELATED_FRAMEWORK_ICONS_ROOT_DIR . '/font-awesome/icons.json' ) );
