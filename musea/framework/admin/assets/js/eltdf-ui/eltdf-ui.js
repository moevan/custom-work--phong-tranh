(function($){
		...
		wp.media.customlibEditGallery1 = {
			frame: function () {
				...
				this._frame.on('update', function () {
					...
					var data = {
						action: 'musea_elated_gallery_upload_get_images',
						ids: ids,
	...
	function eltdfInitAjaxForm() {
		...
		$(document).on("submit", ".eltdf_ajax_form", function (a) {
			var b = $(this),
				c = {
					action: "musea_elated_save_options"
				};
		...
		if (holder.length) {
			...
			eltdfImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Options now?')) {
					...
					var data = {
						action: 'musea_core_import_theme_options',
						content: importValue,
						nonce: importNonce
					};
		...
		if (holder.length) {
			...
			eltdfImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Custom Sidebars now?')) {
					...
					var data = {
						action: 'musea_core_import_custom_sidebars',
						content: importValue,
						nonce: importNonce
					};
		...
		if (holder.length) {
			...
			eltdfImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Widgets now?')) {
					...
					var data = {
						action: 'musea_core_import_widgets',
						content: importValue,
						nonce: importNonce
					};
	...
	function eltdfInitImportContent() {
		...
		if (eltdfImportHolder.length) {
			...
			var eltdfInitContentImport = function(import_expl, progress_bar, progress_value, import_only_content) {
				var xml_file_name = index < 10 ? 'musea_content_0' + index + '.xml' : 'musea_content_' + index + '.xml',
				...
				jQuery.ajax({
					...
					url: ajaxurl,
					data: {
						action: 'musea_core_action_import_content',
						xml: xml_file_name,
						example: import_expl,
						import_attachments: import_attachments
					},
					success: function (data, textStatus, XMLHttpRequest) {
						eltdfSetProgressValue(progress_value, progress_bar, index * 10);
						
						if (index < 10) {
							index++;
							eltdfInitContentImport(import_expl, progress_bar, progress_value, import_only_content);
						} else {
							
							if (import_only_content === true) {
								eltdfSetCompletedMessage(progress_value, progress_bar);
							} else {
								jQuery.ajax({
									type: 'POST',
									url: ajaxurl,
									data: {
										action: 'musea_core_action_import_other_elements',
										example: import_expl
									},
									success: function (data, textStatus, XMLHttpRequest) {
										eltdfSetCompletedMessage(progress_value, progress_bar);
					...
					error: function (data, textStatus, XMLHttpRequest) {
						...
						if (confirm('Some error was happened during the import. Click OK to run it again!')) {
							eltdfImportBtn.trigger('click');
						}
					}
				});
			};
			
			var eltdfSetProgressValue = function(progress_value, progress_bar, progress) {
				progress_value.html((progress) + '%');
				progress_bar.val(progress);
			};
			
			var eltdfSetCompletedMessage = function(progress_value, progress_bar) {
				eltdfSetProgressValue(progress_value, progress_bar, 100);
			...
			eltdfImportBtn.on('click', function (e) {
				...
				if (confirm(confirmMessage)) {
					...
					if (import_opt === 'content') {
						eltdfInitContentImport(import_expl, progress_bar, progress_value, true);
					} else if (import_opt === 'widgets') {
						jQuery.ajax({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'musea_core_action_import_widgets',
								example: import_expl
							},
							success: function (data, textStatus, XMLHttpRequest) {
								eltdfSetCompletedMessage(progress_value, progress_bar);
						...
						jQuery.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'musea_core_action_import_options',
								example: import_expl
							},
							success: function (data, textStatus, XMLHttpRequest) {
								eltdfSetCompletedMessage(progress_value, progress_bar);
						...
						eltdfInitContentImport(import_expl, progress_bar, progress_value);
					}
				}
				return false;
			});
